...
void vbtn_stp(){
  static lv_style_t style_btn_stp;
  lv_style_init(&style_btn_stp);
  lv_style_set_radius(&style_btn_stp, 3);
  lv_style_set_bg_color(&style_btn_stp, lv_palette_main(LV_PALETTE_RED));

  btn_stp = lv_button_create(screen_plotter);
  lv_obj_set_size(btn_stp, 100, 50);
  lv_obj_add_style(btn_stp, &style_btn_stp, 0);
  lv_obj_align(btn_stp, LV_ALIGN_BOTTOM_MID, 0, 0);
  lv_obj_add_event_cb(btn_stp, event_cb_btn_stp, LV_EVENT_CLICKED, NULL);

  lv_obj_t * label_btn_stp = lv_label_create(btn_stp);
  lv_label_set_text(label_btn_stp, "Stop");
  lv_obj_center(label_btn_stp);
...
void vthreeBTN (){
  ...
  lv_obj_t * btn_print = lv_button_create(screen_plotter);
  lv_obj_set_size(btn_print, 100, 50);
  lv_obj_add_style(btn_print, &style_btn, 0);
  lv_obj_align(btn_print, LV_ALIGN_BOTTOM_MID, -150, 0);

  lv_obj_t * label_btn_print = lv_label_create(btn_print);
  lv_label_set_text(label_btn_print, "Print");
  lv_obj_center(label_btn_print);

  lv_obj_t * btn_finish = lv_button_create(screen_plotter);
  lv_obj_set_size(btn_finish, 100, 50);
  lv_obj_add_style(btn_finish, &style_btn, 0);
  lv_obj_align(btn_finish, LV_ALIGN_BOTTOM_MID, 0, 0);
  lv_obj_add_event_cb(btn_finish, event_cb_btn, LV_EVENT_CLICKED, NULL);

  lv_obj_t * label_btn_finish = lv_label_create(btn_finish);
  lv_label_set_text(label_btn_finish, "Finish");
  lv_obj_center(label_btn_finish);
  

  lv_obj_t * btn_flush = lv_button_create(screen_plotter);
  lv_obj_set_size(btn_flush, 100, 50);
  lv_obj_add_style(btn_flush, &style_btn, 0);
  lv_obj_align(btn_flush, LV_ALIGN_BOTTOM_MID, 150, 0);
  lv_obj_add_event_cb(btn_flush, event_cb_btn2, LV_EVENT_CLICKED, NULL);


  lv_obj_t * label_btn_flush = lv_label_create(btn_flush);
  lv_label_set_text(label_btn_flush, "Flush");
  lv_obj_center(label_btn_flush);

  // lv_obj_add_flag(btn_print, LV_OBJ_FLAG_HIDDEN);
  // lv_obj_add_flag(btn_finish, LV_OBJ_FLAG_HIDDEN);
  // lv_obj_add_flag(btn_flush, LV_OBJ_FLAG_HIDDEN);
...
static void event_cb_btn_stp(lv_event_t * e)
{
    LV_UNUSED(e);

    lv_subject_set_int(&fw_update_status_subject, FW_UPDATE_STATE_STOP);
    lv_subject_add_observer_obj(&fw_update_status_subject, fw_update_win_observer_cb, screen_plotter, NULL);
    Serial.println("stop clicked");
    // lv_obj_add_flag(btn_stp, LV_OBJ_FLAG_HIDDEN);
...
static void fw_update_win_observer_cb(lv_observer_t * observer, lv_subject_t * subject)
{
    ...
    if(status == FW_UPDATE_STATE_IDLE) {
      ...
      // lv_obj_del(btn_stp);
      lv_subject_remove_all_obj(&fw_update_status_subject, screen_plotter);
      lv_obj_add_flag(btn_stp, LV_OBJ_FLAG_HIDDEN);
      vthreeBTN();
      // b_btn_stp = true;
      // lv_obj_clear_flag(btn_print, LV_OBJ_FLAG_HIDDEN);
      // lv_obj_clear_flag(btn_finish, LV_OBJ_FLAG_HIDDEN);
      // lv_obj_clear_flag(btn_flush, LV_OBJ_FLAG_HIDDEN);
...
static void event_cb_btn(lv_event_t *e) {
  if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
    ...
    // lv_obj_clear_flag(btn_stp, LV_OBJ_FLAG_HIDDEN);
